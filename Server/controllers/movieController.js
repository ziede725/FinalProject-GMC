...
const getMovieByName = async (req, res, next) => {
  ...
  try {
    const movie = await Movie.findOne({
      movieInfos: { original_title: movieTitle },
    });
    if (!movie) {
      throw new ErrorHandler(
        "404",
        "There are no movies with this title in the database "
      );
    }
    console.log(movie);
    res.status(200).json({
...
const createMovie = async (req, res, next) => {
  ...
  try {
    ...
    const genresID = await Promise.all(
      filterGenres.map(
        async (el) => await Genre.findOne({ name: el[0] }).then((res) => res)
      )
    );
    ...
    id.map(async (el) => {
      try {
        await Genre.findByIdAndUpdate(el, {
          $addToSet: { movieId: newMovie._id },
        });
      } catch (err) {
        console.log(err);
      }
